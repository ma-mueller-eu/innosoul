    ...
    <div style={{
      ...
      <div style={{ padding: '24px 28px 28px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ background: '#F7FAFC', borderRadius: 8, borderLeft: `3px solid ${accentColor}`, padding: '12px 14px 12px 16px', margin: '6px 0' }}>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontStyle: 'normal', fontSize: 13, lineHeight: 1.7, color: '#4A5568' }}>"{bio}"</p>
        </div>
      </div>
    </div>
  );
...
const Team = () => (
  <section id="ueber-uns" data-screen-label="Über uns" style={{ background: '#F7FAFC', padding: '96px 32px' }}>
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 28, maxWidth: 800, margin: '0 auto' }}>
        ...
        <TeamCard
          ...
          initials="NK"
          accentColor="#805AD5"
          photo="../../assets/nicole.jpg"
        />
      </div>
      {/* Values strip */}
      <div style={{ marginTop: 56, display: 'flex', gap: 0, background: '#1A2238', borderRadius: 16, overflow: 'hidden' }}>
        {[
...
Object.assign(window, { Team, TeamCard });
