...
const Contact = () => {
  ...
  const handleFocus = e => { e.target.style.borderColor = '#20C997'; e.target.style.boxShadow = '0 0 0 3px rgba(32,201,151,0.18)'; };
  const handleBlur = e => { e.target.style.borderColor = '#E2E8F0'; e.target.style.boxShadow = 'none'; };

  return (
    <section id="kontakt" data-screen-label="Kontakt" style={{ background: '#F7FAFC', padding: '96px 32px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'start' }}>
        ...
        <div>
          ...
          <button style={{
            ...
            background: '#1A2238', color: 'white', border: 'none',
            borderRadius: 8, padding: '14px 24px', fontFamily: 'DM Sans, sans-serif',
            fontWeight: 600, fontSize: 15, cursor: 'pointer', marginBottom: 32,
          }}>
          ...
          <p style={{ fontFamily: 'DM Sans', fontSize: 14, color: '#A0AEC0' }}>
            Oder per E-Mail: <a href="mailto:hello@innosoul.consulting" style={{ color: '#20C997', textDecoration: 'none', fontWeight: 500 }}>hello@innosoul.consulting</a>
          </p>
        </div>
        {/* Right — Form */}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                ...
                <input style={inputStyle} placeholder="Dr. Max Mustermann" onFocus={handleFocus} onBlur={handleBlur} />
              </div>
              <div>
                <label style={{ fontFamily: 'DM Sans', fontSize: 12, fontWeight: 600, color: '#1A2238', display: 'block', marginBottom: 6 }}>E-Mail *</label>
                <input style={inputStyle} type="email" placeholder="max@company.de" onFocus={handleFocus} onBlur={handleBlur} />
              </div>
            </div>
            <div>
              <label style={{ fontFamily: 'DM Sans', fontSize: 12, fontWeight: 600, color: '#1A2238', display: 'block', marginBottom: 6 }}>Unternehmen</label>
              <input style={inputStyle} placeholder="MedTech GmbH" onFocus={handleFocus} onBlur={handleBlur} />
            </div>
            <div>
              <label style={{ fontFamily: 'DM Sans', fontSize: 12, fontWeight: 600, color: '#1A2238', display: 'block', marginBottom: 6 }}>Ihr Vorhaben *</label>
              <textarea style={{ ...inputStyle, height: 110, resize: 'none' }} placeholder="Kurze Beschreibung Ihres Forschungsvorhabens…" onFocus={handleFocus} onBlur={handleBlur} />
            </div>
            <button style={{
            ...
            <p style={{ fontFamily: 'DM Sans', fontSize: 11, color: '#A0AEC0', textAlign: 'center' }}>Kein Spam. Keine Verpflichtung. Antwort innerhalb von 48 Stunden.</p>
          </div>
        </div>
      </div>
    </section>
